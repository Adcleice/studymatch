...
export default function Chat({ session }) {
  ...
  async function loadChat() {
    ...
    if (matchError || !match) { navigate('/matches'); return; }
        ...
        {messages.length === 0 && (
          <div style={styles.empty}>
            ...
            <p style={{ color: '#6B7280', textAlign: 'center', lineHeight: 1.6 }}>
              Diga olá para {otherUser?.name}!<br />Combinem um horário para estudar juntos.
            </p>
          </div>
        )}
        {messages.map((msg, i) => {
...
const styles = {
  ...
  messages: { flex: 1, overflowY: 'auto', padding: '20px 16px 80px', display: 'flex', flexDirection: 'column' },
  ...
  sendBtn: { background: '#6C63FF', color: 'white', width: 48, height: 48, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'opacity 0.2s' },
};
