...
export default class ErrorBoundary extends React.Component{
  ...
  componentDidCatch(error,info){console.error('Matchworking UI error',error,info)}
  render(){if(!this.state.error)return this.props.children;return <main style={s.page}><section style={s.card}><div style={s.mark}>!</div><h2>Algo não carregou corretamente</h2><p>Seus dados não foram apagados. Recarregue o Matchworking para tentar novamente.</p><button className="btn-primary" onClick={()=>window.location.reload()}>Recarregar aplicativo</button><button style={s.secondary} onClick={()=>{window.location.href='/'}}>Ir para o mapa</button></section></main>}
}
