...
import { Star, MapPin, Plus, X, Search, Camera, Shield, ChevronRight, Trophy } from 'lucide-react';
...
export default function Places({ session }) {
  ...
  const [view, setView] = useState('top10'); // 'top10' | 'byState'
  ...
  const [newPlace, setNewPlace] = useState({ name: '', address: '', city: '', state: '', tags: [], photo_url: '' });
  ...
  async function addPlace() {
    if (!newPlace.name || !newPlace.city || !newPlace.state) return;
    setSaving(true);
    ...
    setNewPlace({ name: '', address: '', city: '', state: '', tags: [], photo_url: '' });
    setShowAdd(false);
    setSubmitted(true);
    setSaving(false);
  }
  ...
  // Data helpers
  const top10 = places.filter(p => p.avg_rating > 0).slice(0, 10);
  const states = Array.from(new Set(places.map(p => p.state).filter(Boolean))).sort();
  const citiesInState = selectedState ? Array.from(new Set(places.filter(p => p.state === selectedState).map(p => p.city).filter(Boolean))).sort() : [];
  const placesInCity = selectedCity ? places.filter(p => p.state === selectedState && p.city === selectedCity) : [];

  const searchResults = search.length > 1 ? places.filter(p =>
    p.name.toLowerCase().includes(search.toLowerCase()) ||
    p.city.toLowerCase().includes(search.toLowerCase())
  ) : [];

  const PlaceCard = ({ place, rank }) => (
    <div style={styles.card} onClick={() => openPlace(place)}>
      <div style={styles.cardImageWrap}>
        ...
        {rank && <div style={styles.rankBadge}>#{rank}</div>}
      </div>
      <div style={styles.cardBody}>
        <div style={styles.cardTop}>
          <h3 style={styles.placeName}>{place.name}</h3>
          <div style={styles.ratingPill}><Star size={13} fill="#FBBF24" color="#FBBF24" /><span style={styles.ratingNum}>{place.avg_rating > 0 ? place.avg_rating.toFixed(1) : '—'}</span></div>
        </div>
        <div style={styles.addressRow}><MapPin size={12} color="#9CA3AF" /><span style={styles.address}>{place.city}{place.state ? `, ${place.state}` : ''}</span></div>
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <div style={styles.headerTop}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {isAdmin && (
              <button style={{ ...styles.addBtn, background: pending.length > 0 ? '#EF4444' : '#9CA3AF', position: 'relative' }} onClick={() => setShowAdmin(true)}>
                <Shield size={18} />
        ...
        <div style={styles.searchBox}>
          ...
          {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', color: '#9CA3AF', cursor: 'pointer' }}>✕</button>}
        </div>
        <div style={styles.viewTabs}>
          <button onClick={() => { setView('top10'); setSelectedState(null); setSelectedCity(null); }}
            style={{ ...styles.viewTab, ...(view === 'top10' ? styles.viewTabActive : {}) }}>
            <Trophy size={14} /> Top 10
          </button>
          <button onClick={() => setView('byState')}
            style={{ ...styles.viewTab, ...(view === 'byState' ? styles.viewTabActive : {}) }}>
            <MapPin size={14} /> Por Estado
          </button>
        </div>
      </div>
      ...
      <div style={styles.list}>
        ...
        {search.length > 1 && (
          <>
            <p style={styles.sectionLabel}>Resultados para "{search}"</p>
            {searchResults.length === 0 && <p style={{ color: '#9CA3AF', fontSize: 14, padding: '8px 0' }}>Nenhum lugar encontrado.</p>}
            {searchResults.map(p => <PlaceCard key={p.id} place={p} />)}
          </>
        )}
          ...
          <>
            {top10.length === 0 && (
              <div style={styles.empty}>
                ...
                <p style={{ fontSize: 13, color: '#9CA3AF' }}>Sugira o primeiro lugar!</p>
                <button className="btn-primary" style={{ marginTop: 8 }} onClick={() => setShowAdd(true)}>+ Sugerir lugar</button>
              </div>
            )}
            {top10.map((p, i) => <PlaceCard key={p.id} place={p} rank={i + 1} />)}
          </>
        )}

        {/* By State */}
        {!search && view === 'byState' && !selectedState && (
          <>
            <p style={styles.sectionLabel}>Selecione um estado</p>
            {states.length === 0 && <p style={{ color: '#9CA3AF', fontSize: 14 }}>Nenhum lugar cadastrado ainda.</p>}
            <div style={styles.stateGrid}>
              {states.map(s => (
                <button key={s} style={styles.stateBtn} onClick={() => setSelectedState(s)}>{s}</button>
              ))}
            </div>
          </>
        )}

        {!search && view === 'byState' && selectedState && !selectedCity && (
          <>
            <div style={styles.breadcrumb}>
              <button onClick={() => setSelectedState(null)} style={styles.breadcrumbBtn}>Estados</button>
              <ChevronRight size={14} color="#9CA3AF" />
              <span style={{ fontWeight: 600 }}>{selectedState}</span>
            </div>
            <p style={styles.sectionLabel}>Cidades em {selectedState}</p>
            {citiesInState.map(c => (
              <button key={c} style={styles.cityBtn} onClick={() => setSelectedCity(c)}>
                <div>
                  <p style={{ fontWeight: 600, fontSize: 14 }}>{c}</p>
                  <p style={{ fontSize: 12, color: '#9CA3AF' }}>{places.filter(p => p.state === selectedState && p.city === c).length} lugar(es)</p>
                </div>
        ...
        {!search && view === 'byState' && selectedState && selectedCity && (
          <>
            <div style={styles.breadcrumb}>
              <button onClick={() => setSelectedState(null)} style={styles.breadcrumbBtn}>Estados</button>
              <ChevronRight size={14} color="#9CA3AF" />
              <button onClick={() => setSelectedCity(null)} style={styles.breadcrumbBtn}>{selectedState}</button>
              <ChevronRight size={14} color="#9CA3AF" />
              <span style={{ fontWeight: 600 }}>{selectedCity}</span>
            </div>
            {placesInCity.length === 0 && <p style={{ color: '#9CA3AF', fontSize: 14, padding: '16px 0' }}>Nenhum lugar nesta cidade ainda.</p>}
            {placesInCity.map((p, i) => <PlaceCard key={p.id} place={p} rank={i + 1} />)}
          </>
        )}
      </div>
          ...
          <div style={styles.modal}>
            ...
            {submitted ? (
              <div style={{ padding: '20px 20px 32px', textAlign: 'center' }}>
                ...
                <p style={{ color: '#6B7280', fontSize: 14, lineHeight: 1.6 }}>Sua sugestão foi enviada para aprovação!</p>
              ...
              <div style={styles.form}>
                ...
                <select value={newPlace.state} onChange={e => setNewPlace(p => ({ ...p, state: e.target.value }))}>
                ...
                <p style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>⏳ Será avaliado antes de aparecer publicamente.</p>
                <button className="btn-primary" onClick={addPlace} disabled={saving || uploading || !newPlace.name || !newPlace.city || !newPlace.state}>
      ...
      {showAdmin && isAdmin && (
        <div style={styles.overlay}>
          <div style={{ ...styles.modal, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>🛡️ Admin ({pending.length} pendentes)</h3>
            ...
            <div style={{ padding: '0 20px 24px' }}>
              ...
              {pending.map(p => (
                <div key={p.id} style={{ background: '#F9FAFB', borderRadius: 12, padding: 14, marginBottom: 12 }}>
                  ...
                  <p style={{ color: '#6B7280', fontSize: 13 }}>📍 {p.address}, {p.city} - {p.state}</p>
        ...
        <div style={styles.overlay}>
          <div style={{ ...styles.modal, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '16px 20px 32px' }}>
              ...
              {selected.address && <div style={{ ...styles.addressRow, marginBottom: 12 }}><MapPin size={13} color="#6C63FF" /><span style={{ fontSize: 13, color: '#6B7280' }}>{selected.address}, {selected.city} - {selected.state}</span></div>}
...
const styles = {
  ...
  addBtn: { background: '#6C63FF', color: 'white', width: 42, height: 42, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative', border: 'none', cursor: 'pointer' },
  ...
  viewTabs: { display: 'flex', gap: 8, paddingBottom: 12 },
  viewTab: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px', borderRadius: 10, border: '1.5px solid #E5E7EB', background: 'white', color: '#6B7280', fontSize: 13, fontWeight: 500, cursor: 'pointer' },
  viewTabActive: { background: '#6C63FF', color: 'white', borderColor: '#6C63FF' },
  list: { padding: '12px 16px' },
  sectionLabel: { fontSize: 13, fontWeight: 700, color: '#6C63FF', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 },
  empty: { textAlign: 'center', padding: 48, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, color: '#6B7280' },
  stateGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 16 },
  stateBtn: { background: 'white', border: '1.5px solid #E5E7EB', borderRadius: 10, padding: '12px 8px', fontWeight: 700, fontSize: 14, color: '#374151', cursor: 'pointer' },
  breadcrumb: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, fontSize: 14 },
  breadcrumbBtn: { background: 'none', border: 'none', color: '#6C63FF', fontWeight: 600, cursor: 'pointer', fontSize: 14, padding: 0 },
  cityBtn: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', border: 'none', borderRadius: 12, padding: '14px 16px', width: '100%', marginBottom: 8, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', textAlign: 'left' },
